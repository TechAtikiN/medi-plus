...
import medicines from '../utils/meds.json'

const Tablets = () => {
  const [inputMed, setInputMed] = useState('')
  const [selectedMed, setSelectedMed] = useState<MedicineItem | null>()

  const items: MedicineItem[] = Object.values(medicines.Name).map((item, index) => {
    return {
      name: item,
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>, item: MedicineItem) => {
    e.preventDefault()
    setSelectedMed(item)
  }
        ...
        <div className='bg-indigo-50 h-screen'>
          ...
          {selectedMed && (
            <div className='absolute space-y-4 p-4 z-50 text-center bg-indigo-400 text-white flex flex-col h-3/5 w-1/2 top-40 right-80'>
              ...
              <p><span className='font-bold mx-2 text-indigo-900 text-lg'>Side Effects:</span> {selectedMed.sideEffects}</p>
              <p><span className='font-bold mx-2 text-indigo-900 text-lg'>Tips:</span> {selectedMed.quickTips.slice(0, 500)}...</p>
              <button onClick={() => setSelectedMed(null)} className='w-1/6 mx-auto bg-indigo-700 my-5 font-semibold text-white rounded-full p-2'>Close</button>
            </div>
          )}
