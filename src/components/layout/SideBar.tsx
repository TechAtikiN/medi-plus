...
const SideBar = () => {
  ...
  return (
    <>
      <div className="sm:bg-violet-700 sm:w-80 flex flex-col">
          ...
          {navLinks.map((link) => (
            <div
              ...
              className={` hover:font-bold font-semibold flex space-x-3 px-3 py-2 cursor-pointer 
                       hover:bg-gray-100 hover:text-violet-800  m-2
                       ${link.path === selectedItem ? "text-violet-700 bg-gray-100 " : ""}
                       `}
            >
            <Link href={ `/${link.path}`} className="flex">
            <Bars3Icon className="h-6 w-6 text-gray-200 hover:text-violet-700 mr-3" />
            <p className={`text-sm hover:text-violet-700 text-gray-200 text-left `}>{link.title}</p>
            </Link>
            </div>
